...
export function DptAddModal({
	...
	const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		...
		const validate = (d: object) => {
			const dptAdd = z.object({
				jenisKelamin: z
					.string()
					.min(1, { message: "Jenis Kelamin Harus Di pilih" }),
			});

			const { error } = dptAdd.safeParse(d);
			if (error) {
						...
						<form className="space-y-3" onSubmit={onSubmit}>
							...
							<div className="mb-4 md:flex md:justify-between gap-3">
								...
								<div className="w-full md:w-1/2">
									...
									<InputErrorMessage error={errors["jenisKelamin"]} />
								</div>
							</div>
