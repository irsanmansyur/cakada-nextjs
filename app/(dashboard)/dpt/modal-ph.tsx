...
export default function ModalFPH({
	...
	const [open, setOpen] = useState(!!dpt);
	...
	return (
		...
		>
			<div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-40 backdrop-blur w-screen p-4">
				<DialogPanel className="max-w-3xl max-h-[90%] overflow-y-auto space-y-4 border rounded bg-white p-4 shadow">
					<DialogTitle className="font-bold flex justify-between items-center">
						Input Program Harapan
...
function ModalDfhLocal({
	...
	return (
		<form className="w-full space-y-4" onSubmit={handleSubmit}>
			<div className="flex">
				<label className="sm:w-[300px] w-1/2 flex-shrink-0">Kecamatan</label>
				<input
					type="text"
			...
			<div className="flex">
				...
				<div>
					<input
						...
						className="input input-bordered w-full max-w-xs input-sm"
						value={data.kelName}
						onChange={(e) => setData({ ...data, kelName: e.target.value })}
					/>
				</div>
			</div>
			<div className="flex">
			...
			<div className="flex flex-col sm:flex-row">
				<label className="pb-1 w-[300px] flex-shrink-0">Kepala Keluarga</label>
				<div className="w-full">
					<KepalaKeluargaSelect
						...
						value={
							{
								value: data?.kepalaKeluargaId,
								label: data?.kepalaKeluarga,
							} as any
						}
						onChange={(v) => {
							setData((e) => ({
			...
			<ul className="space-y-4 pt-4">
							...
							<div className="form-control">
								<label className="label cursor-pointer justify-normal gap-2">
									<input
										...
										checked={
											data.harapan === "Seragam Gratis Siswa Baru (SD/SMP)"
										}
										onChange={(e) => {
											setData({
												...data,
								...
								<label className="label cursor-pointer justify-normal gap-2">
									...
									<div className="flex gap-2 items-center">
										...
										<input
											...
											value={formatValuePilihanLainya(data.harapan, "Lainya")}
											onChange={(e) => {
												setData({
													...data,
				...
				<li className="flex gap-2">
					<span>2. </span>
					<div>
						Apakah Bapak/Ibu akan mendukung jika <strong>{namaCaleg} </strong>
							...
							<div className="form-control">
								<label className="label cursor-pointer justify-normal gap-2">
									<input
										...
										checked={data.mendukung.includes("Mendukung Kandidat Lain")}
										onChange={(e) => {
											setData({
												...data,
												mendukung: e.target.value,
			...
			<div className="flex gap-4">
				<div className="w-1/2">
					<FormGroup
						...
						onChange={(e) => setData({ ...data, namaRelawan: e.target.value })}
						classNameParent="w-full"
